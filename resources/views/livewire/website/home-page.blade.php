<div>
    <a href="{{ route('website.spcialAd', ['id' => $data['SpcialAd']['spcial_ad_id'], app()->getLocale()]) }}">
        <div>
            <!-- ======= Hero Section ======= -->
            @if (file_exists(storage_path('app/img/' . $data['SpcialAd']['picture'])))
                <div id="hero" class="hero route bg-image"
                    style="background-image: url('data:image/png|jpg|jpeg;base64, {!! base64_encode(file_get_contents(storage_path('app/img/' . $data['SpcialAd']['picture']))) !!}'); width: 100%;
                                height: 50vw;
                                object-fit: fill;">
                @else
                    <div id="hero" class="hero route bg-image"
    ...
    <div class="container my-5">
        <div id="carouselExampleDark" class="carousel carousel-dark slide" data-bs-ride="carousel">
            <div class="carousel-indicators">
                @forelse ($data['SpcialAdNG'] as $key => $ads)
                    ...
                    <div class="carousel-item {{ $key == 0 ? 'active' : '' }}" data-bs-interval="10000">
                        <a href="{{ route('website.spcialAd', ['id' => $ads->spcial_ad_id, app()->getLocale()]) }}">
                            ...
                            <div class="carousel-caption d-md-block">
                                @if (app()->getLocale() == 'en')
                                    <h5 class="text-white">{{ $ads->en_title ?? '' }}</h5>
                                    <p class="text-white">{{ $ads->en_desc ?? '' }}</p>
                                @else
                                    <h5 class="text-white">{{ $ads->ar_title ?? '' }}</h5>
                                    <p class="text-white">{{ $ads->ar_desc ?? '' }}</p>
                                @endif
                            </div>
                        </a>
                    </div>
                                                            ...
                                                            <div class="media">
                                                                <div class="media-body text-center">
                                                                    <img src="{{ asset('img/home.svg') }}"
                                                                        alt="icon" width="150px"><br />
                                                                    <span
                                                                        class="mt-3">{{ __('Real Estate') }}</span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
