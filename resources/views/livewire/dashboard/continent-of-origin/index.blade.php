...
@section('tableName', __('Continent Of Origin'))
            ...
            <tr>
                ...
                <td>
                    <button type="button" class="btn btn-outline-primary mb-2" data-bs-toggle="modal"
                        data-bs-target="#editModal" wire:click.prevent="edit({{ $data->continent_id }})"><i
...
@section('create-model-body')
    @if (Session()->has('WrongContinentOfOrigin'))
        <div class="alert alert-danger">
            {{ Session('WrongContinentOfOrigin') }}
        </div>
    @endif
    @if (!Session()->has('WrongContinentOfOrigin'))
        <div class="card-body">
                    ...
                    <div class="col-md-6">
                        <div class="form-floating mb-3">
                            <input class="form-control" id="inputname" type="text" name="en_name" autocomplete="off"
                                wire:model="en_name" />
                            <label for="inputen_name">English Name
                            </label>
                    ...
                    <div class="col-md-6">
                        <div class="form-floating mb-3">
                            <input class="form-control" id="inputname" type="text" name="ar_name" autocomplete="off"
                                wire:model="ar_name" />
                            <label for="inputar_name">Arabic Name
                            </label>
                            <span class="text-danger">
                                @error('ar_name')
                                    {{ $message }}
...
@section('edit-model-body')
    ...
    @if (!Session()->has('WrongContinentOfOrigin'))
        <div class="card-body">
            <form autocomplete="off" method="POST">
                @csrf
                <input type="hidden" name="continent_id" wire:model="continent_id">
                <div class="results">
                    @if (Session()->has('message'))
                        <div class="alert alert-success">
                            {{ Session('message') }}
