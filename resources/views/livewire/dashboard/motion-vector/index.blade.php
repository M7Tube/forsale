            ...
            <tr>
                ...
                <td>
                    <button type="button" class="btn btn-outline-primary mb-2" data-bs-toggle="modal"
                        data-bs-target="#editModal" wire:click.prevent="edit({{ $data->motion_vector_id }})"><i
...
@section('create-model-body')
    @if (Session()->has('WrongMotionVector'))
        <div class="alert alert-danger">
            {{ Session('WrongMotionVector') }}
        </div>
    @endif
    @if (!Session()->has('WrongMotionVector'))
        <div class="card-body">
...
@section('edit-model-body')
    ...
    @if (!Session()->has('WrongMotionVector'))
        <div class="card-body">
            <form autocomplete="off" method="POST">
                @csrf
                <input type="hidden" name="motion_vector_id" wire:model="motion_vector_id">
                <div class="results">
                    @if (Session()->has('message'))
                        <div class="alert alert-success">
                            {{ Session('message') }}
