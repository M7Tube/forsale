...
class ApiFilterController extends Controller
{
    ...
    public function view($lang, $per_page, $category)
    {
        ...
        if ($category == 1) { //cars category
            ...
            if ($lang == 'ar') {
                //filter resualt
                $filterResualt = Cars::where('manger_accept', 2)->filter()->paginate($per_page, ['car_id', 'ar_title', 'ar_desc', 'phone_number', 'picture', 'is_special', 'price', 'manger_accept', 'governorate_id', 'created_at']);
                //count ads
                ...
                //filter resualt
                $filterResualt = Cars::where('manger_accept', 2)->filter()->paginate($per_page, ['car_id', 'en_title', 'en_desc', 'phone_number', 'picture', 'is_special', 'price', 'manger_accept', 'governorate_id', 'created_at']);
                //count ads
                $count = Cars::where('manger_accept', 2)->filter()->count();
                //get the filters
                $filters = [
